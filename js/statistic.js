...
function initEventListeners() {
  ...
  document.addEventListener("click", (event) => {
    ...
    const isClickOnKeyword =
      event.target.closest(".keyword-item") ||
...
async function loadPapersByDateRange(startDate, endDate) {
  ...
  try {
    ...
    allTitle.forEach((abstract, index) => {
      const date =
        validDatesInRange[
          Math.floor(index / (allTitle.length / validDatesInRange.length))
        ];
        ...
        <div class="statistics-card">
          <div class="keyword-list">
            ${keywordCloudData
              .map(
                (item, index) => `
              <div class="keyword-item" onclick="showRelatedPapers('${
                item.text
              }')">
    ...
    if (startDate !== endDate) {
      ...
      const width =
        document.getElementById("trendChart").offsetWidth -
        margin.left -
        margin.right;
      ...
      if (startDate.getFullYear() === endDate.getFullYear()) {
        if (startDate.getMonth() === endDate.getMonth()) {
          xAxisTitle = `${startDate.getFullYear()}/${String(
      ...
      legend
        ...
        .on("mouseout", function () {
          ...
          paths.style("opacity", 0.8).style("stroke-width", 2);
        });
    }
  } catch (error) {
...
function showRelatedPapers(keyword) {
  ...
  // 生成相关论文的HTML
  const papersHTML = relatedPapers
    .map(
      (paper, index) => `
  ...
  relatedPapersContainer.innerHTML =
    relatedPapers.length > 0 ? papersHTML : "<p>No related papers found.</p>";
