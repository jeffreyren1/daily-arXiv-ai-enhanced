...
function renderKeywordTags() {
  ...
  userKeywords.forEach((keyword) => {
    ...
    tagElement.className = `category-button ${
      activeKeywords.includes(keyword) ? "active" : ""
    }`;
...
function renderAuthorTags() {
  ...
  userAuthors.forEach((author) => {
    const tagElement = document.createElement("span");
    tagElement.className = `category-button ${
      activeAuthors.includes(author) ? "active" : ""
    }`;
...
function initEventListeners() {
  ...
  document
    ...
    .addEventListener("change", toggleRangeMode);
...
function parseJsonlData(jsonlText, date) {
  ...
  lines.forEach((line) => {
    try {
      ...
      let allCategories = Array.isArray(paper.categories)
        ? paper.categories
        : [paper.categories];
...
function renderCategoryFilter(categories) {
  ...
  container.innerHTML = `
    ...
    }" data-category="all">All<span class="category-count">${totalPapers}</span></button>
  `;
...
function renderPapers() {
    ...
    filteredPapers.forEach((paper) => {
      const matchesKeyword =
        activeKeywords.length > 0
          ? activeKeywords.some((keyword) => {
              const searchText =
                `${paper.title} ${paper.summary}`.toLowerCase();
              return searchText.includes(keyword.toLowerCase());
            })
          : false;
  ...
  filteredPapers.forEach((paper, index) => {
    ...
    paperCard.innerHTML = `
      ...
      ${
        paper.isMatched
          ? '<div class="match-badge" title="匹配您的搜索条件"></div>'
          : ""
      }
...
function showPaperDetails(paper, paperIndex) {
  ...
  modalTitle.innerHTML = paperIndex
    ? `<span class="paper-index-badge">${paperIndex}</span> ${highlightedTitle}`
    : highlightedTitle;
  ...
  const modalContent = `
    <div class="paper-details ${matchedPaperClass}">
      ...
      <div class="paper-sections">
        ${
          paper.motivation
            ? `<div class="paper-section"><h4>Motivation</h4><p>${highlightedMotivation}</p></div>`
            : ""
        }
        ${
          paper.method
            ? `<div class="paper-section"><h4>Method</h4><p>${highlightedMethod}</p></div>`
            : ""
        }
        ${
          paper.result
            ? `<div class="paper-section"><h4>Result</h4><p>${highlightedResult}</p></div>`
            : ""
        }
        ${
          paper.conclusion
            ? `<div class="paper-section"><h4>Conclusion</h4><p>${highlightedConclusion}</p></div>`
            : ""
        }
      </div>
      ...
      ${
        highlightedAbstract
          ? `<h3>Abstract</h3><p class="original-abstract">${highlightedAbstract}</p>`
          : ""
      }
  ...
  prompt = `请你阅读这篇文章${paper.url.replace(
    "abs",
    "pdf"
  )},总结一下这篇文章解决的问题、相关工作、研究方法、做了什么实验及其结果、结论，最后整体总结一下这篇文章的内容`;
  document.getElementById(
    "kimiChatLink"
  ).href = `https://www.kimi.com/_prefill_chat?prefill_prompt=${prompt}&system_prompt=你是一个学术助手，后面的对话将围绕着以下论文内容进行，已经通过链接给出了论文的PDF和论文已有的FAQ。用户将继续向你咨询论文的相关问题，请你作出专业的回答，不要出现第一人称，当涉及到分点回答时，鼓励你以markdown格式输出。&send_immediately=true&force_search=false`;

  // 更新论文位置信息
  const paperPosition = document.getElementById("paperPosition");
  if (paperPosition && currentFilteredPapers.length > 0) {
    paperPosition.textContent = `${currentPaperIndex + 1} / ${
      currentFilteredPapers.length
    }`;
  }
...
function navigateToPreviousPaper() {
  ...
  currentPaperIndex =
    currentPaperIndex > 0
      ? currentPaperIndex - 1
      : currentFilteredPapers.length - 1;
...
function navigateToNextPaper() {
  ...
  currentPaperIndex =
    currentPaperIndex < currentFilteredPapers.length - 1
      ? currentPaperIndex + 1
      : 0;
  const paper = currentFilteredPapers[currentPaperIndex];
  showPaperDetails(paper, currentPaperIndex + 1);
...
function showRandomPaper() {
  ...
  console.log(
    `Showing random paper: ${randomIndex + 1}/${currentFilteredPapers.length}`
  );
}
...
function toggleView() {
  ...
  document
    .getElementById("paperContainer")
    .classList.toggle("list-view", currentView === "list");
}
...
async function loadPapersByDateRange(startDate, endDate) {
  ...
  container.innerHTML = `
    <div class="loading-container">
      <div class="loading-spinner"></div>
      <p>Loading papers from ${formatDate(startDate)} to ${formatDate(
    endDate
  )}...</p>
    </div>
  `;
  ...
  try {
    ...
    for (const date of validDatesInRange) {
      ...
      Object.keys(dataPapers).forEach((category) => {
        ...
        allPaperData[category] = allPaperData[category].concat(
          dataPapers[category]
        );
      });
    }
...
function togglePdfSize(button) {
  const pdfContainer = button
    .closest(".pdf-preview-section")
