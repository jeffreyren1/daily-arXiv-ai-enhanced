...
function addKeywordTag(keyword) {
  ...
  // 检查关键词是否已存在
  const existingTags =
    selectedKeywordsContainer.querySelectorAll(".category-button");
...
function addAuthorTag(author) {
  ...
  // 检查作者是否已存在
  const existingTags =
    selectedAuthorsContainer.querySelectorAll(".category-button");
  for (let i = 0; i < existingTags.length; i++) {
  ...
  removeButton.addEventListener("click", (e) => {
    ...
    setTimeout(() => {
      ...
      // 如果没有标签了，显示空标签消息
      if (
        selectedAuthorsContainer.querySelectorAll(".category-button").length ===
        0
      ) {
        showEmptyAuthorMessage();
      }
...
function saveSettings() {
  // 获取所有选中的关键词
  const keywordTags = document
    .getElementById("selectedKeywords")
...
function showNotification(message, type = "success") {
  ...
  if (type === "success") {
    ...
    icon =
      '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 15c-.55 0-1-.45-1-1v-4c0-.55.45-1 1-1s1 .45 1 1v4c0 .55-.45 1-1 1zm1-8h-2V7h2v2z" fill="currentColor"/></svg>';
    bgColor = "#3b82f6";
  }
